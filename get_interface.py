...
def get_interfaces():
    ...
    resp = requests.get(f'{api_url}{module}',
    ...
    if resp.status_code == 200:
        ...
        for key, valor in data_json.items():
            ...
            print(
                f'Descripción de la interface: {valor["interface"][0]["description"]}')
            print(
                f'Status de la interface: {valor["interface"][0]["enabled"]}')
    else:
...
def put_banner():
    ...
    module = "data/Cisco-IOS-XE-native:native/banner/motd"
    resp = requests.put(f'{api_url}{module}', data=json.dumps(
        banner), auth=basicauth, headers=headers, verify=False)
...
def post_loopback():
    ...
    module = "data/ietf-interfaces:interfaces"
    resp = requests.post(f'{api_url}{module}', auth=basicauth,
                         headers=headers, data=dloopback, verify=False)
...
def del_loopback():
    ...
    resp = requests.delete(f'{api_url}{module}', auth=basicauth,
                           headers=headers, data=dloopback, verify=False)
    if resp.status_code == 204:
